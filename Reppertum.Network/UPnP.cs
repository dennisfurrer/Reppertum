...
namespace Reppertum.Network
{
    public class UPnP
    {
        ...
        {
            Int32 n = resp.IndexOf("://");
            n = resp.IndexOf('/', n + 3);
            return resp.Substring(0, n) + p;
        }

        public static void CreateForwardingRule(Int32 port, ProtocolType protocol, string description)
        {
        ...
        public static void DeleteForwardingRule(Int32 port, ProtocolType protocol)
        {
            if (string.IsNullOrEmpty(_serviceUrl)) throw new Exception("No UPnP service available or Discover() has not been called");
            XmlDocument xdoc = SOAPRequest(_serviceUrl,
